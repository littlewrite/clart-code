...
- MCP 高级传输（SSE/HTTP/WebSocket）与 OAuth 认证（当前仅 stdio）。
...
- Dart 侧：已升级为”可运行迁移基座（Iteration 10 级别）”，核心功能 80% 完整。
- 迁移策略：继续按能力域增量落地，始终保持主程序可运行。
- 代码质量：MVP+ 级别实现完成，架构清晰，类型安全，测试覆盖良好。
- MCP 基础：已实现 stdio 传输、工具/资源桥接、服务器管理。

## D. 下一步优先级（2026-04-05 后续）

### P0（关键）
- [x] 添加单元测试覆盖核心模块（query_engine, turn_executor, providers）
- [x] 实现 Tool 并发分组调度（当前仅串行）
- [x] 完善错误恢复机制

### P1（重要）
- [x] 实现 Task 后台任务系统（基础版）
- [x] 添加 MCP 基础连接（stdio 传输）
- [x] 完善权限细粒度控制
